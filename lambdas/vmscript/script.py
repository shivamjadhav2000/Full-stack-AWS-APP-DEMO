...
def terminate_instance(instance_name):
    # Get the first instance with the specified name tag
    ...
    # Extract the instance ID of the first instance found
    instance_id = None
    ...
    if not instance_id:
        print("No instance found with the name:", instance_name)
        return

    # Terminate the instance
    print("Terminating the instance with the name:", instance_name)
    ec2.terminate_instances(InstanceIds=[instance_id])

    print("Instance terminated successfully.")   

def main(*args):
    record_id,table_name,instance_name = args
    try:
